...
<script setup lang="ts">
    ...
    import {
        createGlobalEditCheckAPI,
        deleteGlobalEditCheckByIdAPI,
        globalEditCheckListPaginationAPI,
        updateGlobalEditCheckAPI,
    } from "~/services/apis/handlers/admin/global-edit-check";
    import type { IGlobalEditCheckData } from "~/services/apis/handlers/types/admin/global-edit-check-types";
    ...
    const groupLabel = computed(() => "Edit Checks");
    ...
    async function deleteEditCheckyId(editCheckId: string) {
        ...
        const { status, message } = await deleteGlobalEditCheckByIdAPI(editCheckId);
        if (status === 200) {
            toastStore.success({
                title: "Success",
                message,
    ...
    function setTableData(data: IGlobalEditCheckData[]): ITableData[] {
    ...
    async function editCheckPaginationList() {
        ...
        const { status, data, message } = await globalEditCheckListPaginationAPI(
            getAPIFilters({
    ...
    async function saveAddOrEditCheck(rowIndex: number) {
        ...
        if (!checkIfObjectHasErrors(validationErrors.value)) {
            ...
            if (!tableData.value[rowIndex].id) {
                ...
                } = await createGlobalEditCheckAPI(requestData);
                message = apiMessage;
                status = apiStatus;
                ...
                const {
                    ...
                    status: apiStatus,
                    validationErrors: apiValidationErrors,
                } = await updateGlobalEditCheckAPI(requestData);
                message = apiMessage;
                status = apiStatus;
                validationErrorsData = apiValidationErrors;
            }
                                ...
                                <template #action="{ data, index }">
                                        ...
                                        :on-click="
                                            () => {
                                                emit('on-row-select', {
                                                    ...
                                                    module: 'Global Edit Checks',
                                                    page: 'Global Edit Checks Entries',
                                                    data,
                                                });
                                            }
                                        "
                                    ...
                                    <edc-delete-confirmation
                                        v-if="
                                            ...
                                            !disableAction
                                        "
                                        :key="`${checkIfKeyExistsInObject(index, tableLoaderDetails) && tableLoaderDetails[index] ? 'show' : 'hide'}DeleteDictionaryEntryConfirmation${data.tempID}`"
