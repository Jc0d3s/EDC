<script setup lang="ts">
    ...
    import {
        dictionaryEntriesPaginationListAPI,
        createDictionaryAPI,
        updateDictionaryAPI,
        dictionaryEntriesCreateAPI,
        dictionaryEntriesUpdateAPI,
        dictionaryPaginationListAPI,
        reorderEntryAPI,
        deleteDictionaryByIdAPI,
        deleteDictionaryEntryByIdAPI,
    } from "~/services/apis/handlers/admin/dictionary";
    import type {
        IDictionaryEntryData,
        IDictionaryData as IDictionaryDataAPI,
    } from "~/services/apis/handlers/types/admin/dictionary-types";
    ...
    const dictionaryGroupLabel = computed(() =>
        checkIfKeyExistsInObject(props.activeTab.label, props.tabData)
            ? `Dictionaries - ${props.tabData[props.activeTab.label].data.branchName}`
            : "Dictionaries",
    );
    ...
    function setDictionaryTableData(tableData: IDictionaryDataAPI[]) {
    ...
    async function dictionaryPaginationList() {
        ...
        const { status, data, message } = await dictionaryPaginationListAPI(
            getAPIFilters({
    ...
    async function saveAddOrEditDictionaryApi(rowIndex: number) {
        ...
        if (!checkIfObjectHasErrors(dictionaryValidationErrors.value)) {
            ...
            if (!dictionaryTableData.value[rowIndex].id) {
                ...
                } = await createDictionaryAPI(requestData);
                message = apiMessage;
                status = statusMessage;
                ...
                const {
                    ...
                    status: statusMessage,
                    validationErrors: apiValidationErrors,
                } = await updateDictionaryAPI(requestData);
                message = apiMessage;
                status = statusMessage;
                validationErrorsData = apiValidationErrors;
            }
    ...
    async function deleteDictionaryById(dictionaryId: string) {
        ...
        const { status, message } = await deleteDictionaryByIdAPI(dictionaryId);
        if (status === 200) {
            toastStore.success({
                title: "Success",
                message,
    ...
    function setTableData(tableData: IDictionaryEntryData[]): IEntryData[] {
    ...
    async function dictionaryEntryPaginationList() {
        ...
        const { status, data, message } = await dictionaryEntriesPaginationListAPI(
            getAPIFilters({
    ...
    async function saveAddOrEditDictionaryEntry(rowIndex: number) {
        ...
        if (!checkIfObjectHasErrors(dictionaryEntryValidationErrors.value)) {
            ...
            const requestData = {
                ...
                // order: entryTableData.value[rowIndex].order,
            ...
            if (!entryTableData.value[rowIndex].id) {
                const {
                    ...
                    status: statusMessage,
                    validationErrors: apiValidationErrors,
                } = await dictionaryEntriesCreateAPI(requestData);
                message = apiMessage;
                status = statusMessage;
                validationErrorsData = apiValidationErrors;
            } else {
                const {
                    ...
                    status: statusMessage,
                    validationErrors: apiValidationErrors,
                } = await dictionaryEntriesUpdateAPI(requestData);
                message = apiMessage;
                status = statusMessage;
                validationErrorsData = apiValidationErrors;
            }
    ...
    async function deleteDictionaryEntryById(dictionaryEntryId: string) {
        loading.value.form = true;
        const { status, message } = await deleteDictionaryEntryByIdAPI(dictionaryEntryId);
        if (status === 200) {
            toastStore.success({
                title: "Success",
                message,
    ...
    const {
        isModelChanged: isDictionaryModelChanged,
        model: dictionaryModel,
        modelRef: dictionaryModelRef,
    } = useUnsavedAlert(dictionaryTableData.value, dictionaryTableDataRef.value);
    const {
        isModelChanged: isDictionaryEntryModelChanged,
        model: entryModel,
    ...
    watch(
        () => isDictionaryModelChanged.value,
        (newValue) => {
    ...
    watch(
        () => isDictionaryEntryModelChanged.value,
        (newValue) => {
            if (newValue) unsavedAlertStore.triggerFormActionAlert = "unsaved";
            else unsavedAlertStore.triggerFormActionAlert = "none";
        },
    ...
    watch(
        ...
        (newTriggerFormActionAlert) => {
            if (newTriggerFormActionAlert === "reset" && isDictionaryModelChanged.value) {
                dictionaryTableData.value = deepClone(dictionaryTableDataRef.value);
                resetDictionaryEditFormDetails();
