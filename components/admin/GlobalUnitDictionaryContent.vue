<script setup lang="ts">
    ...
    import {
        globalUnitDictionaryCreateAPI,
        globalUnitDictionaryDeleteByIdAPI,
        globalUnitDictionaryEntriesCreateAPI,
        globalUnitDictionaryEntriesPaginationListAPI,
        globalUnitDictionaryEntriesUpdateAPI,
        globalUnitDictionaryEntryDeleteByIdAPI,
        globalUnitDictionaryPaginationListAPI,
        globalUnitDictionaryUpdateAPI,
        reorderGlobalUnitDictionaryEntryAPI,
    } from "~/services/apis/handlers/admin/global-unit-dictionary";
    import type {
        IGlobalUnitDictionaryData,
        IGlobalUnitDictionaryEntryData,
    } from "~/services/apis/handlers/types/admin/global-unit-dictionary-types";
    ...
    function setUnitDictionaryTableData(tableData: IGlobalUnitDictionaryData[]) {
        ...
        return tableData.map((row) => ({
            ...
            tempID: uniqueID(),
            updatedBy: 0,
        })) as IGlobalUnitDictionaryData[];
    }

    async function unitDictionaryPaginationList() {
        loading.value.table = true;
        const { status, data, message } = await globalUnitDictionaryPaginationListAPI(
            getAPIFilters({
    ...
    async function saveAddOrEditUnitDictionary(rowIndex: number) {
        ...
        if (!checkIfObjectHasErrors(unitDictionaryValidationErrors.value)) {
            ...
            if (!unitDictionaryTableData.value[rowIndex].id) {
                ...
                } = await globalUnitDictionaryCreateAPI(requestData);
                message = apiMessage;
                status = statusMessage;
                ...
                const {
                    ...
                    status: statusMessage,
                    validationErrors: apiValidationErrors,
                } = await globalUnitDictionaryUpdateAPI(requestData);
                message = apiMessage;
                status = statusMessage;
                validationErrorsData = apiValidationErrors;
            }
    ...
    async function deleteUnitDictionaryById(unitDictionaryId: string) {
        ...
        const { status, message } = await globalUnitDictionaryDeleteByIdAPI(unitDictionaryId);
        if (status === 200) {
            toastStore.success({
                title: "Success",
                message,
    ...
    function setTableData(tableData: IGlobalUnitDictionaryEntryData[]): IEntryData[] {
    ...
    async function unitDictionaryEntriesPaginationList() {
        ...
        const { status, data, message } = await globalUnitDictionaryEntriesPaginationListAPI(
            getAPIFilters({
        ...
        if (!checkIfObjectHasErrors(unitDictionaryEntryValidationErrors.value)) {
            ...
            if (!entryTableData.value[rowIndex].id) {
                const {
                    ...
                    status: statusMessage,
                    validationErrors: apiValidationErrors,
                } = await globalUnitDictionaryEntriesCreateAPI(requestData);
                message = apiMessage;
                status = statusMessage;
                validationErrorsData = apiValidationErrors;
            } else {
                const {
                    ...
                    status: statusMessage,
                    validationErrors: apiValidationErrors,
                } = await globalUnitDictionaryEntriesUpdateAPI(requestData);
                message = apiMessage;
                status = statusMessage;
                validationErrorsData = apiValidationErrors;
            }
    ...
    const getReorderRequestBody = (): IGlobalUnitDictionaryEntryData[] =>
        entryTableData.value.map((row) => ({
            ...
            formulaValueOne: 0,
            formulaValueTwo: 0,
        })) as IGlobalUnitDictionaryEntryData[];

    async function reorderEntries() {
        loading.value.dynamicLoading.reOrder = true;
        const { status, message } =
            await reorderGlobalUnitDictionaryEntryAPI(getReorderRequestBody());
        if (status === 200) {
            toastStore.success({
                title: "Success",
                message,
    ...
    async function deleteUnitDictionaryEntryById(unitDictionaryEntryId: string) {
        loading.value.form = true;
        const { status, message } =
            await globalUnitDictionaryEntryDeleteByIdAPI(unitDictionaryEntryId);
        if (status === 200) {
            toastStore.success({
                title: "Success",
                message,
    ...
    const { model: unitDictionaryModel, modelRef: unitDictionaryModelRef } = useUnsavedAlert(
        unitDictionaryTableData.value,
        unitDictionaryTableDataRef.value,
    );
...
<template>
    <div class="grid grid-cols-1 w-full gap-[1rem]">
        <admin-card-wrapper group-label="Unit Dictionaries">
            <template #content>
